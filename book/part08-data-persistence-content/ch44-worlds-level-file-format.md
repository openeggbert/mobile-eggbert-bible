...
![Reconstructed collision/classification map of world001.txt's 100×100 Decor grid](../images/level-map-world001.png)

*Reconstructed from real level data, not a screenshot.* This diagram renders every cell of
`worlds/world001.txt`'s `Decor:` block — the same 100×100 grid whose raw comma-separated rows are
quoted above — classified by `Decor`'s own `Is*()` collision predicates and `Tables::table_decor_quart`,
using the exact parsing rules this section describes (`-1` for an empty slot). It is generated by
`tools/render_level_map.py` directly from the shipped level file, not captured from a running build;
see `book/images/MANIFEST.md` for the full generation method and two more worked examples
(`world055.txt`, `world120.txt`).
